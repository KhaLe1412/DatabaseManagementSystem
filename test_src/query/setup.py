...
def create_server():
    print("=" * 60)
    print("🐳 KHỞI TẠO DOCKER SERVER (Query Benchmark)")
    print("=" * 60)

    commands = [
        ("Dọn dẹp container cũ (nếu có)", "docker rm -f mysql_benchmark redis_benchmark"),
        ("Khởi động MySQL container",
         "docker run -d --name mysql_benchmark -p 3306:3306 "
         "-e MYSQL_ROOT_PASSWORD=123456 -e MYSQL_DATABASE=sales_benchmark mysql:8.0"),
...
def _run_sql_file(file_path: Path):
    ...
    command = ["docker", "exec", "-i", "mysql_benchmark", "mysql", "-uroot", "-p123456"]
    try:
        result = subprocess.run(
            command, input=sql_script, text=True,
...
def init_redis():
    ...
    try:
        ...
        if commands_file.exists():
            import shlex
            print(f"🔄 Loading Redis data from: {commands_file.name}")
            with commands_file.open(encoding="utf-8") as f:
                for raw in f:
                    ...
                    try:
                        ...
                        print(f"❌ Redis command failed: '{line}' -> {e}")
