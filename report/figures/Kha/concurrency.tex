...
Redis hoạt động Single-threaded, mọi lệnh của Redis (INCR, HINCRBY, v.v.), các transaction (MULTI / EXEC), Lua Script đều nguyên tử và được thực thi tuần tự. Điều này đảm bảo rằng không có 2 lệnh nào chạy đồng thời, do đó các transaction được xử lý tuần tự, không gặp race-condition, không phải dùng các cơ chế lock phức tạp.
...
\begin{itemize}
    ...
    \item Khi ít thread, Redis nhanh hơn, nhưng khi nhiều thread, MySQL có xu hướng vượt trội hơn.
\end{itemize}
...
\subsubsection{Test đọc ghi hỗn hợp}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/Kha/rw3thread.png}
    \caption{Kết quả test đọc ghi hỗn hợp theo tỉ lệ}
    \label{fig:mixedtest}
\end{figure}
...
\textbf{Nhận xét: }\\
\begin{itemize}
    \item MySQL vẫn cho kết quả tốt khi lượng đọc vẫn còn ít, càng đọc nhiều thì throughput càng giảm.
    \item Redis cho throughput tốt và ổn định dù ở tỉ lệ nào.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Tính Repeatable Read của MySQL nhờ MVCC}
    \label{fig:mvcctest}
\end{figure}

\textbf{Nhận xét:}
\begin{itemize}
    \item \textbf{Điểm tạo Snapshot:} Ngay khi Reader thực hiện lệnh SELECT đầu tiên (hoặc BEGIN), một "bản chụp" dữ liệu tại thời điểm đó được xác lập.
    \item \textbf{Độc lập với Writers:} Sau khi snapshot được tạo, Reader sẽ chỉ nhìn thấy dữ liệu tại thời điểm đó, bất kể Writer có thực hiện UPDATE/INSERT/DELETE nào sau đó. Điều này đảm bảo tính Repeatable Read.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/Kha/mvccmul_reader.png}
    \caption{Quản lý đa phiên bản (Multi-versioning) trong MVCC}
    \label{fig:mvcctest2}
\end{figure}
\textbf{Nhận xét: }
\begin{itemize}
    \item \textbf{Đa phiên bản:} MySQL không chỉ giữ một phiên bản mà có thể quản lý nhiều phiên bản cùng lúc
    \item \textbf{Phiên bản dữ liệu:} Mỗi khi Writer thực hiện UPDATE, một phiên bản mới của hàng được tạo ra. Reader sẽ chỉ nhìn thấy phiên bản tại thời điểm snapshot được tạo, không bị ảnh hưởng bởi các phiên bản mới.
\end{itemize}

\textbf{Ưu điểm về hiệu năng: } Reader không bị block bởi Writer và ngược lại, giúp tăng hiệu năng khi có nhiều transaction đọc và ghi đồng thời.
